...
function crossHopNotes(h) {
  ...
  /* ★取引所と言えなくても、調べた結果そのものが手がかりになる。
     実測（利用者提供・XRP→TRON／MistTrackで実照会）：この地点で名前が
     付いた取引相手は ChangeNow の 0.016% だけで、残りはすべて素性不明だった。
     ＝取引所の入金アドレスではなく、資金を混ぜる中継点。
     ★「見つかりませんでした」で終えると、調べた事実まで消えてしまう。
     まだ換金されていない可能性を示す情報なので、そのまま書く。 */
  if (!h.cpInferred && Array.isArray(h.counterparty)) {
    const top = h.counterparty[0];
    out.push(note('cross-cp-none', 'info', 'この地点は取引所の入金アドレスではないと考えられます',
      '取引の相手を調べたところ、'
      + (top ? `名前が付いた相手は ${top.name}（${top.percent}%）のみで、` : '名前の付く相手が無く、')
      + '残りはすべて素性が分かっていません。'
      + '取引所の入金アドレスであれば、その取引所との取引が大半を占めます。',
      'この資金はまだ換金されていない可能性があります。'
      + 'このアドレス自体を、被害届・凍結要請の対象としてご相談ください。'));
  }
  if (h.cpInferred) {
...
function resultNotes(result, paid) {
  ...
  if (!paid) {
    /* ★level を 'good' から 'afterbuy' に変える。'good' は
       「到達した取引所」「ブリッジ」「分散した送金先」＝★判明した事実の色で、
    ...
    out.push(note('afterbuy', 'afterbuy', 'ご購入後の流れ',
      ...
      /* ★但し書きは text 側に置く。sub は『強調して添える一行』として作られており
         （例：宛先タグの注記の「必ず併記してください」）、色が本文より明るい。
         細字のつもりで sub に入れたら、箱の中でいちばん目立つ文字になっていた（実測）。 */
