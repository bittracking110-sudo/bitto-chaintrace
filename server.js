...
app.post('/api/connection/consult', express.json(), async (req, res) => {
  try {
    ...
    if (ctx && ctx.chain) {
      ...
      if (exNames) {
        exState = `判明（${exNames}）`;
        freezeLetter = '作成できます（宛先の取引所が判明しているため）';
      } else if (ctx.reachedExchange) {
        exState = '推定（取引所系とみられるウォレットに到達したが、名称は特定できていない）';
        freezeLetter = '作成できません（宛先の取引所を特定できないため）';
      } else {
    ...
    if (GEMINI_KEY) {
      ...
      const refundBlock = asksRefund
        ? `【この質問は「返金の可能性」に関するものです。次を各1行ずつ簡潔に必ず含めてください】
1. 銀行振込の詐欺で使われる「振り込め詐欺救済法（被害回復分配法）」は、**預金口座を対象とした制度**です。暗号資産交換業者の口座は預金口座にあたらないため、**暗号資産はこの制度の対象外**であり、法律に基づいて自動的に返金・分配される仕組みは存在しません。
2. そのうえで、現実的に取り得る道すじ：
   ① 記録の保全（TXID、相手とのやり取り、送金履歴、相手の情報のスクリーンショット）
   ② 資金の追跡と、到達した取引所の特定
   ③ 特定できた取引所への連絡・凍結の要請（**凍結するかどうかは取引所の判断**であり、確約はできません）
   ④ **警察への相談・被害届**（各都道府県警のサイバー犯罪相談窓口、または最寄りの警察署）
3. 弁護士について：ご相談は選択肢の一つです。ただし暗号資産は上記のとおり返金分配の法的枠組みが無いため、**対応できる範囲が限られる場合がある**ことを事実として伝えます。**弁護士を否定・非難する表現は使わず**、優先順位として「取引所への要請」と「警察への相談」を先に案内してください。
4. 最後に、上の【ご相談者の追跡結果】に当てはめて、今回どうすべきかを1〜2文で述べる。
※ これらは制度の一般的な説明であり、個別の法律判断ではありません。`
...
【回答の作り方（厳守）】
- **挨拶・お礼・自己紹介を書かない**。「ご相談ありがとうございます」「BitToの調査AIです」等で始めず、1文目から本題に入る
- **聞かれた質問にまっすぐ答える**。聞かれていないことを長々と述べない
- **追跡結果を再説明しない**（利用者の画面に既に表示されています）。触れる場合は1文まで
- 質問された語をそのまま受けて答える（別の話題にすり替えない）
- 上の指示ブロックに書かれた内容は、指示どおり必ず反映する
- 箇条書きを使い、**全体で450文字以内を厳守**（超えそうなら要点を削る）。日本語
...
【回答の作り方（厳守）】
- **挨拶・お礼・自己紹介を書かない**。「この度はご相談いただき…」等で始めず、1文目から本題に入る
- **聞かれた質問にまっすぐ答える**。聞かれていないことを長々と述べない
- **追跡結果を再説明しない**（利用者の画面に既に表示されています）。触れる場合は1文まで
- 質問された語をそのまま受けて答える（別の話題にすり替えない）
- 上の指示ブロックに書かれた内容は、指示どおり必ず反映する
